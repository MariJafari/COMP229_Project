...
export  async function ProcessFindProductsByKeyword(req: express.Request, res: express.Response, next: express.NextFunction) :void
{
//   try {
...
//     if (!keyword) {
//         res.status(400).send({ error: 'Keyword parameter is missing' });
//         return;
//     }

//     // Finding products that contain the keyword in their name
//     Product.find({ name: { $regex: new RegExp(keyword) } }, function (err: any, matchingProducts: any[]) {
//         if (err) {
//             console.error(err);
...
//pass the id to the database and delete the product
Product.findById({_id:id}, function(err: CallbackError){
