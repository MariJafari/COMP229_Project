...
export function ProcessFindProductsByKeyword(req: express.Request, res: express.Response, next: express.NextFunction) :void
{
  ...
  Product.find({
    ...
    if (err) {
      console.error(err);
      res.status(500).render('error', { title: 'Error', message: 'Error finding products by keyword.', error: err.message });
    } else {
      res.render('product-search-results', { title: 'Search Results', page: 'product-search-results', products: products, keyword: keyword });
    }
  });
